	...
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-md-12">
				<div class="card">
					<div class="card-body">
						<h3 class="card-title">员工查询</h3>
						<!-- 模糊查询的搜索表单 -->
						<!-- 跳转页面的表单 -->
						<div>
							<div class="col-md-3"></div>
							<form id="admin_Q_sta" action="admin_Q_sta">
								<div class="col-md-3">
						...
						<div class="table-responsive">
							<table class="table">
								...
								<tbody>
									<!-- 遍历集合 -->
									<c:forEach items="${adm_staffList}" var="adm_Q_sta">
										<tr>
											<th scope="row" style="height: 50px;">${adm_Q_sta.id}</th>
											<td>${adm_Q_sta.name}</td>
											<td>${adm_Q_sta.password}</td>
											<td>${adm_Q_sta.card}</td>
											<td>${adm_Q_sta.sex}</td>
											<th><figure class="avatar avatar-sm"> <img
													src="/image/${adm_Q_sta.head}" class="rounded-circle" style="height: 30px;width: 30px;">
												</figure></th>
											<td>${adm_Q_sta.pos_name}</td>
											<td>${adm_Q_sta.dep_name}</td>
											<td>${adm_Q_sta.phone}</td>
											<td>${adm_Q_sta.time}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
